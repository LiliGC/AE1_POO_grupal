...
from xlo import limpiarpantalla
...
class Vendedor:
    ...
    def vender():
        Clientes.mostrar_id()
        cliente_compra= input("\nSeleccione un cliente para realizar la compra (1-5): ")
...
while True:
    ...
    if opcion == 1:
        while True:
            ...
            # 1.1 Mostrar saldo de usuario elegido:
            if opcion1 == 1:
                limpiarconsola()
                Clientes.mostrar_id()
            ...
            # 1.2 Agregar saldo a usuario elegido:
            elif opcion1 == 2:
                cliente_consultar= input("Seleccione un cliente del 1 al 5 que quiere ver su saldo: ")
            ...
            # 1.4 Mostrar todos los saldos de los clientes
            elif opcion1 == 4:
    ...
    # 2. Productos.
        ...
        while True:
            ...
            # 2.2 Modificar unidades de stock
            elif opcion2 == 2:
                limpiarconsola()
                print("Nuestros productos:\n")
                for i in range(len(productos)):
                    print("{:<5}{:25}{:5}".format(i + 1, pv[i].nombre.capitalize(), pv[i].stock))
                n = int(input("\nSeleccione producto: "))
                mod = input(f"\nUd. ha seleccionado el producto {pv[n - 1].nombre.capitalize()}.\nIngrese el número de unidades a modificar seguido de + o - (agregar o quitar)... ")
                mod1 = mod[-1]
                if mod1 == "+":
                    pv[n - 1].agregar_stock(int(mod[:-1]))
                    print(f"El nuevo stock del producto {pv[n - 1].nombre.capitalize()} es de {pv[n - 1].stock} unidad(es)")
                else:
                    pv[n - 1].descontar_stock(int(mod[:-1]))
            
                suma = 0    
                for i in range(len(productos)):
                    suma += pv[i].stock
                print(f"Suma total de unidades de todos los productos: {suma}")
                
                if suma <= 500:
                    print("Capacidad de sucursal OK")
                input()
